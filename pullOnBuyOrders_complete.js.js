// ============================================
// SHARED ORDER PULL LOGIC (used by HTTP + Scheduler)
// ============================================

// NOTE: If you already have this at the top of index.js, delete this line:
const functions = require("firebase-functions");

async function runPullOnBuyOrdersLogic() {
  ...
  if (orgSnap.empty) {
    return { imported: 0, updated: 0, skipped: 0, message: "No orgs configured" };
  }
  ...
  for (const orgDoc of orgSnap.docs) {
    ...
    if (!account.onbuyApiKey) {
      logger.info(account.name + ": no API key");
      continue;
    }
    ...
    for (const o of orders) {
      ...
      if (!onbuyOrderId) {
        logger.warn("Skipping order with no ID");
        continue;
      }

      const docId = "onbuy_" + String(onbuyOrderId);
...
// ============================================
// HTTP ENDPOINT — Manual trigger via URL
// ============================================

exports.pullOnBuyOrders = onRequest({ timeoutSeconds: 300, memory: "1GiB" }, async (req, res) => {
  try {
    const result = await runPullOnBuyOrdersLogic();
...
// ============================================
// CLOUD SCHEDULER — Auto-runs every 60 minutes
// ============================================

exports.scheduledPullOnBuyOrders = functions
  .runWith({ timeoutSeconds: 300, memory: "1GiB" })
  .pubsub.schedule("every 60 minutes")
  .timeZone("Europe/London")
  .onRun(async (context) => {
    const logger = require("firebase-functions").logger;
    logger.info("⏰ Scheduled pullOnBuyOrders triggered at: " + new Date().toISOString());

    try {
      const result = await runPullOnBuyOrdersLogic();
      logger.info("✅ Scheduled run completed: " + JSON.stringify(result));
      return null;
    } catch (err) {
