...
function parseDisputeEmail(subject, text, html, messageId) {
  ...
  // Extract dispute reference (e.g., T6K27YN, T6KQVBS) from subject or body
  const refMatch = subject.match(/([A-Z0-9]{6,8})/) ||
                   body.match(/Reference[:\s#]+([A-Z0-9]{6,8})/i) ||
                   body.match(/dispute ref[:\s#]+([A-Z0-9]{6,8})/i) ||
                   body.match(/ref[:\s]+([A-Z0-9]{6,8})/i);
  if (refMatch) result.disputeRef = refMatch[1];

  // Extract order ID — try multiple patterns
  const orderPatterns = [
    /Order\s*#?\s*([A-Z0-9]{6,10})/i,
    /order[_\s]id[:\s]+([A-Z0-9]{6,10})/i,
    /order\s+ref[:\s]+([A-Z0-9]{6,10})/i,
    /your OnBuy order\s+([A-Z0-9]{6,10})/i,
    /OnBuy order\s+([A-Z0-9]{6,10})/i,
    /order number[:\s]+([A-Z0-9]{6,10})/i,
  ];
  for (const p of orderPatterns) {
    const m = body.match(p);
    if (m) { result.orderId = m[1]; break; }
  }
  // Fallback to subject
  if (!result.orderId) {
    const subOrder = subject.match(/([A-Z0-9]{6,8})/);
    if (subOrder) result.orderId = subOrder[1];
  }
  ...
  async (req, res) => {
    try {
      ...
      if (parsed.messageId) {
        ...
        if (!existing.empty) {
          logger.info(`Dispute duplicate skipped (messageId): ${parsed.messageId}`);
          return res.json({ success: true, duplicate: true, disputeId: existing.docs[0].id });
        }
      }
    ...
    for (const buf of msgs) {
      try {
        ...
        if (disputeData.messageId) {
          ...
          if (!existing.empty) {
            logger.info(`IMAP: skipping duplicate message ${disputeData.messageId}`);
            continue;
          }
        }
        // Secondary dedup: disputeRef + orderId (catches pre-messageId docs)
        if (disputeData.disputeRef && disputeData.orderId) {
          const existing2 = await db.collection('orderTracker_disputes')
            ...
            .limit(1).get();
          if (!existing2.empty) {
            logger.info(`IMAP: skipping duplicate dispute ${disputeData.disputeRef} / ${disputeData.orderId}`);
            continue;
          }
        }
