// ============================================
// LIVE DATA EXPORT — FIXED VERSION
// ============================================
...
exports.getLiveData = onRequest({cors: true}, async (req, res) => {
  ...
  try {
    // --- ORDERS (recent only for pending count) ---
    ...
    res.json({
      ...
      orders: {
        ...
        pendingDispatch: {
          recent: pendingDispatch,
          stale: stalePending,
          panacea: panaceaPending,
          samayy: samayyPending,
          note: 'Recent = last 7 days only. Stale = older orders probably already dispatched on OnBuy.'
        },
